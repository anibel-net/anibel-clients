...
{
    public Func<string, object?, CancellationToken, Task<object?>>? Handler
    {
        get; set;
    }
    public List<(string Op, JsonElement Args)> Calls { get; } = [];
    public UpdatesPageDto UpdatesPage { get; set; } = new([], 0, false);
    public MediaKindDto KindChoice { get; set; } = new("episodes", ["notselected", "watching", "watched", "dropped", "planned"]);
    public EpisodeChoicesDto EpisodeChoices { get; set; } = new([], [], "dub", []);
    public Anibel.App.Services.SessionSnapshot Session { get; set; } = new(0, false, null, null, null);
    public async Task<T> CallAsync<T>(string op, object? args = null, CancellationToken ct = default)
    {
        ct.ThrowIfCancellationRequested();
        Calls.Add((op, JsonSerializer.SerializeToElement(args)));
        object? result = Handler is null ? op switch
        {
            "updatesPage" => UpdatesPage,
            "mediaKind" => KindChoice,
            "episodeChoices" => EpisodeChoices,
            "session" => Session,
            "searchHistory" => Array.Empty<string>(),
            "playbackReport" => new Anibel.App.Playback.PlaybackActionDto(null),
            _ => throw new InvalidOperationException($"No test response for {op}"),
        } : await Handler(op, args, ct);
        return (T)result!;
    }
    ...
    public Exception? MediaListError
    {
        get; set;
    }
    ...
    public Task<List<MediaCard>> SearchAsync(string query, int limit = 10, CancellationToken ct = default)
    {
        ...
        if (SearchError is not null)
            return Task.FromException<List<MediaCard>>(SearchError);
    ...
    public int LastMediaListLimit
    {
        get; private set;
    }
    public int MediaListDelayMs
    {
        get; set;
    }
    ...
    public Task<PlaybackIntentDto> ResolveEpisodeAsync(string videoIdOrUrl, CancellationToken ct = default)
        => Task.FromResult(new PlaybackIntentDto(PlaybackKind.Native, null, null, "https://n/x", null, null, [], [], 1));
    ...
    public string? LastAddedCommentReplyTo
    {
        get; private set;
    }
    public Func<Task<CommentDto>>? CommentHandler { get; set; }
    public int AddCommentCalls { get; private set; }
    public Task<CommentDto> AddCommentAsync(string mediaId, string mediaType, string content, string? replyTo = null, CancellationToken ct = default)
    {
        ...
        return CommentHandler?.Invoke() ?? Task.FromResult(new CommentDto { Id = "new", Content = content });
    }
    ...
    public string? LastMarkAsStatus
    {
        get; private set;
    }
    public string? LastRemoveMarkStatus
    {
        get; private set;
    }
...
public sealed class MemoryCredentialStore : Anibel.App.Services.ICredentialStore
{
    public LoginUserDto? Value
    {
        get; set;
    }
    public Task<LoginUserDto?> ReadAsync() => Task.FromResult(Value);
    public Task WriteAsync(LoginUserDto value)
    {
        Value = value;
        return Task.CompletedTask;
    }
