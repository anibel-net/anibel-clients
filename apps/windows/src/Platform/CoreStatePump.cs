...
internal sealed class CoreStatePump
{
    ...
    private async Task RefreshAsync()
    {
        try
        {
            foreach (var notification in _core.DrainEvents())
                if (notification.TryGetProperty("e", out var kind) && kind.GetString() is "error" or "cache.storageError")
                    Diag.Log($"Core reported {kind.GetString()}; refreshing host snapshots.");
            ...
            _timer.Interval = TimeSpan.FromMilliseconds(_downloads.Items.Any(i => i.Status is DownloadStatus.Queued or DownloadStatus.Downloading) ? 500 : 3000);
        }
