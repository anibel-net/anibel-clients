...
public partial class MediaDetailsViewModel : ObservableObject
{
    ...
    {
        var kind = await _core.CallAsync<MediaKindDto>(CoreCommand.MediaKind, new { mediaType = type });
    ...
    public async Task SetRatingAsync(int rating)
    {
        ...
        try
        {
            await _core.CallAsync<System.Text.Json.JsonElement>(CoreCommand.SetRating, new { mediaId = media.MediaId, mediaType = media.MediaType, rating });
    ...
    private async Task UpdateEpisodeChoicesAsync()
    {
        ...
        try
        {
            var choices = await _core.CallAsync<EpisodeChoicesDto>(CoreCommand.EpisodeChoices, new { mediaId = Media.MediaId, kind = SelectedKind, resource });
    ...
    public async Task SetFavoriteAsync(bool want)
    {
        ...
        try
        {
            var result = await _core.CallAsync<SelectionResult>(CoreCommand.SetFavorite, new { mediaId = media.MediaId, mediaType = media.MediaType, selected = want });
    ...
    public async Task ChangeMarkAsync()
    {
        ...
        try
        {
            var result = await _core.CallAsync<MarkResult>(CoreCommand.SetMark, new { mediaId = media.MediaId, mediaType = media.MediaType, status, current = media.Mark?.Status });
            if (!ReferenceEquals(Media, media) || revision != _session.Revision) return;
    ...
    public async Task ToggleWatchedAsync(EpisodeDto? ep)
    {
        ...
        try
        {
            var result = await _core.CallAsync<SelectionResult>(CoreCommand.SetWatched, new { entityId = ep.Id, selected = ep.Watched != true });
