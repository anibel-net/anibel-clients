...
{
    private readonly MediaDownloadActions _downloadActions;
    ...
    public MediaDetailsPage()
    {
        ...
        InitializeComponent();
        _downloadActions = new MediaDownloadActions(Vm, App.Services.GetRequiredService<DownloadService>(), ShowDownloadNote, ShowDownloadError);
    ...
    private async void OnDownloadEpisodeClick(object sender, RoutedEventArgs e)
    {
        if (sender is Button { Tag: EpisodeDto ep })
        {
            await _downloadActions.QueueEpisode(ep, audioOnly: false);
        }
    }
    ...
    private async void OnDownloadEpisodeAudioClick(object sender, RoutedEventArgs e)
    {
        if (sender is Button { Tag: EpisodeDto ep })
        {
            await _downloadActions.QueueEpisode(ep, audioOnly: true);
        }
    }

    private async void OnDownloadAllEpisodesClick(object sender, RoutedEventArgs e) => await _downloadActions.QueueAllEpisodes();

    private async void OnDownloadChapterClick(object sender, RoutedEventArgs e)
    {
        if (sender is Button { Tag: ChapterDto ch })
        {
            await _downloadActions.QueueChapter(ch);
        }
    }

    private async void OnDownloadAllChaptersClick(object sender, RoutedEventArgs e) => await _downloadActions.QueueAllChapters();

    private async void OnDownloadMediaFileClick(object sender, RoutedEventArgs e) => await _downloadActions.QueueFile();
