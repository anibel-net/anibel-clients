...
internal fun ProfileScreen(isTv: Boolean, username: String? = null, edgeToEdge: Boolean = false) {
    ...
    val profile = if (signedIn || username != null) coreData("profile", json("username" to username), revision) else null
    ...
    PageColumn(onRefresh = if (isTv) null else { { revision++ } },
        ...
        if (!signedIn && username == null) {
            ...
            AppButton(if (action.busy) ui(R.string.signing_in) else ui(R.string.sign_in), isTv, {
                run { core.value("login", json("username" to login.trim(), "password" to password)); password = ""; revision++ }
            ...
            if (username == null && signedIn && isTv) {
                AppButton(ui(R.string.sign_out), true, { run { core.value("logout"); revision++ } }, enabled = !action.busy)
            }
        }
            ...
            Column {
                ...
                if (signedIn) {
                    ...
                    ListItem(headlineContent = { Text(ui(R.string.sign_out), color = MaterialTheme.colorScheme.error) },
                        modifier = Modifier.clickable(enabled = !action.busy) { run { core.value("logout"); revision++ } })
                }
            }
        }
    }
...
internal fun ProfileEditor(profile: JSONObject, isTv: Boolean, saved: () -> Unit) {
    ...
    AppButton(ui(R.string.save_profile), isTv, { run {
        ...
        withContext(NonCancellable) { core.value("updateProfile", patch) }
...
internal fun PersonalScreen(kind: String, isTv: Boolean, mediaType: String? = null, showHeader: Boolean = true) {
    ...
    var revision by remember { mutableIntStateOf(0) }
    val data = coreData("personalList", json("kind" to kind), revision)
...
internal fun SettingsScreen(isTv: Boolean) {
    ...
    val (action, run) = rememberAction()
    var revision by remember { mutableIntStateOf(0) }
    val version = coreData("version", revision = revision)
    if (!isTv) {
        PageColumn(onRefresh = if (isTv) null else { { revision++ } }, refreshing = revision > 0 && version == RemoteData.Loading) {
            ...
            Column {
                ListItem(headlineContent = { Text(ui(R.string.clear_catalog_cache)) }, modifier = Modifier.clickable(enabled = !action.busy) {
                    run { core.value("clearCache"); action.message = ui(R.string.catalog_cache_cleared) }
                })
                ListItem(headlineContent = { Text(ui(R.string.clear_playback_files)) }, supportingContent = { Text(ui(R.string.downloads_kept)) },
                    modifier = Modifier.clickable(enabled = !action.busy) {
                        run { core.value("clearPlaybackAssets"); action.message = ui(R.string.playback_files_cleared) }
                    })
            }
