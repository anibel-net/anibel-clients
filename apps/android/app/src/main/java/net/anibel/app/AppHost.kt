...
internal fun AppHost(isTv: Boolean, content: @Composable () -> Unit) {
    ...
    LaunchedEffect(core, lifecycle) {
        lifecycle.repeatOnLifecycle(Lifecycle.State.STARTED) {
            try {
                if (core.call("downloads").optJSONArray("items").objects().any { it.optString("status") in listOf("queued", "downloading") })
            ...
            while (true) {
                try {
                    val next = core.call("session")
                    if (next.toString() != session.toString()) session = next
                } catch (cancelled: CancellationException) { throw cancelled }
                catch (_: Exception) { /* Page requests expose connection errors with retry. */ }
                delay(1000)
            }
        }
    }
                ...
                Column(Modifier.fillMaxSize().background(background).safeDrawingPadding().testTag("screen_${screen.name}")) {
                    ...
                    when (screen) {
                        Screen.Collection -> if (args.optString("op") == "updatesPage")
                            PagedTitles("updatesPage", json("type" to args.optString("type", "ALL")), isTv)
...
@Composable
internal fun coreData(op: String, args: JSONObject = JSONObject(), revision: Int = 0): RemoteData {
