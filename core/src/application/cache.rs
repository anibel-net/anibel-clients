...
pub(super) fn ttl(op: &str) -> Option<(Duration, Duration)> {
    let (fresh, keep) = match op {
        "comments" => (15 * 60, 2 * 86400),
        "profile" | "user" | "me" | "favorites" | "marks" | "status" | "profileHub"
        | "personalList" => (10 * 60, 2 * 86400),
        "updates" | "updatesPage" | "slider" | "trends" => (6 * 3600, 7 * 86400),
        "episodeChoices" | "search" | "media" | "mediaList" | "episodes" | "episodesMatrix"
        | "chapters" | "chapter" | "recommendations" | "schedule" | "filters" | "statistics" => {
            (12 * 3600, 14 * 86400)
        }
        _ => return None,
    };
